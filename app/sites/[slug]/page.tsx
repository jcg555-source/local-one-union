...
export default async function SiteDetailPage({
  ...
  const { site, source, error } = await getPublicSiteBySlug(slug);

  if (!site) {
    notFound();
  }

  const showHiringAlertForm = source === "supabase" && Boolean(site.id);
        ...
        {showHiringAlertForm ? (
          ...
          <div className="card-panel p-6">
            ...
            <p className="mt-3 text-sm leading-7 text-union-steel">
              Hiring alerts are unavailable for this fallback site record until
              the live Supabase site entry is connected.
            </p>
          </div>
        )}
      </div>
